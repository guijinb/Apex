import { hashPassword, sanitize, jsonResponse, validateEmail, validateUsername, validatePassword, checkRateLimit } from '../_utils.js';
...
export async function onRequestPost(context) {
  try {
    ...
    if (!rate.allowed) {
      return jsonResponse({ success: false, message: '注册过于频繁，请稍后再试' }, 429);
    }
    ...
    if (!validatePassword(password)) return jsonResponse({ success: false, message: '密码需至少 8 位，含大小写字母、数字和特殊字符' }, 400);
